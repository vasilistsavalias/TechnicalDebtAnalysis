...
public class InventoryUI extends Window implements InventorySubject, InventorySlotObserver {
    ...
    public InventoryUI() {
        ...
        InventorySlot headSlot = new InventorySlot(InventoryItem.ItemUseType.ARMOR_HELMET.getValue(), new Image(ITEMS_TEXTURE_ATLAS.findRegion("inv_helmet")));
        InventorySlot chestSlot = new InventorySlot(InventoryItem.ItemUseType.ARMOR_CHEST.getValue(), new Image(ITEMS_TEXTURE_ATLAS.findRegion("inv_chest")));
        InventorySlot legsSlot = new InventorySlot(InventoryItem.ItemUseType.ARMOR_FEET.getValue(), new Image(ITEMS_TEXTURE_ATLAS.findRegion("inv_boot")));
        ...
        InventorySlot leftArmSlot = new InventorySlot(weapon_value, new Image(ITEMS_TEXTURE_ATLAS.findRegion("inv_weapon")));
        InventorySlot rightArmSlot = new InventorySlot(weapon_value, new Image(ITEMS_TEXTURE_ATLAS.findRegion("inv_shield")));

        slots.add(headSlot);
        slots.add(chestSlot);
        slots.add(legsSlot);
        slots.add(leftArmSlot);
        slots.add(rightArmSlot);
    ...
    public static Array<InventoryItemLocation> getInventory(Table targetTable, String name) {
        ...
        for(int i = 0; i < cells.size; i++) {
            ...
            if(numItems > 0) {
                //System.out.println("[i] " + i + " itemtype: " + inventorySlot.getTopInventoryItem().getItemTypeID().toString() + " numItems " + numItems);
                items.add(new InventoryItemLocation(i, inventorySlot.getTopInventoryItem().getItemTypeID().toString(), numItems, name));
            }
        }
        return items;
    }
    ...
    public void onNotify(InventorySlot slot, SlotEvent event) {
        event.PerformInventoryUIAction(slot, this);
    }
    ...
    public Label getAPValLabel() {
        return APValLabel;
    }

    public Label getDPValLabel() {
        return DPValLabel;
    }
